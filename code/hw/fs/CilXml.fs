    ...
    let writeToVerilog (typeDefTable : TypeDefTableRec [], methodDefRows : MethodDefRowType []) = 
        ...
        for i in 1..typeDefTable.Length - 1 do
            ...
            typeDefStr <- typeDefStr + "\n" + "// " + typeDefRec.TypeNameStr + "\n" + 
                        "tables.cit[" + (sprintf "%04d" (typeDefMemWordCnt+0)) + "]" + " <= " + "32'h" + (sprintf "%08X" typeDefRec._index) + ";" 
                          + "      // CIT_ID_INDEX" + "\n" + 
                        "tables.cit[" + (sprintf "%04d" (typeDefMemWordCnt + 1)) + "]" + " <= " + "32'h" + (sprintf "%08X" 0) + ";" 
                          + "      // CIT_NUMFIELDS_INDEX" + "\n" + 
                        "tables.cit[" + (sprintf "%04d" (typeDefMemWordCnt + 2)) + "]" + " <= " + "32'h" + (sprintf "%08X" methodDefRows.Length) + ";" 
        ...
        for i in 0..methodDefRows.Length - 1 do
            ...
            methodNamesDefStr <- methodNamesDefStr + "tables.mit_names[" + (sprintf "%08X" methodDefRec._index) + "]" + " <= " + "\"" + methodDefRec.TypeDefTableRecOwner.TypeNameStr
            ...
            methodDefStr <- methodDefStr + "\n" + "// " + methodDefRec.TypeDefTableRecOwner.TypeNameStr 
                            ...
                            "tables.mit[" + (sprintf "%04d" (methodDefMemWordCnt + 0)) + "]" + " <= " + "32'h" + (sprintf "%08X" methodDefRec._index) + ";" 
                                + "      // CIT_MIT_ID_INDEX\n" +
                            "tables.mit[" + (sprintf "%04d" (methodDefMemWordCnt + 1)) + "]" + " <= " + "32'h" + (sprintf "%08X" mitCitKey) + ";" 
                                + "      // CIT_MIT_CITID_INDEX (foreign [key])" + "\n" + 
                            "tables.mit[" + (sprintf "%04d" (methodDefMemWordCnt + 2)) + "]" + " <= " + "32'h" + (sprintf "%08X" methodDefRec.NumArgs) + ";" 
                                + "      // CIT_MIT_NUMARGS_INDEX\n" +
                            "tables.mit[" + (sprintf "%04d" (methodDefMemWordCnt + 3)) + "]" + " <= " + "32'h" + (sprintf "%08X" numLocals) + ";" 
                                + "      // CIT_MIT_NUMLOCALS_INDEX\n" +
                            "tables.mit[" + (sprintf "%04d" (methodDefMemWordCnt + 4)) + "]" + " <= " + "32'h" + (sprintf "%08X" 8) + ";" 
                                + "      // CIT_MIT_MAXEVALSTACK_INDEX\n" +
                            "tables.mit[" + (sprintf "%04d" (methodDefMemWordCnt + 5)) + "]" + " <= " + "32'h" + (sprintf "%08X" methodDefRec.Instructions.Length) + ";" 
                                + "      // CIT_MIT_NUMCIL_INDEX\n" +
                            "tables.mit[" + (sprintf "%04d" (methodDefMemWordCnt + 6)) + "]" + " <= " + "32'h" + (sprintf "%08X" firstCilOffset) + ";" 
        ...
        for i in 0..methodDefRows.Length - 1 do
            ...
            for j in 0..methodDefRec.Instructions.Length - 1 do
                ...
                instDefStr <- instDefStr + "\n" + "// " + methodDefRec.TypeDefTableRecOwner.TypeNameStr 
                                ...
                                "tables.cil[" + (sprintf "%04d" (cilIndexCnt+0)) + "]" + " <= " + "32'h" + (sprintf "%08X" instDefRec.Pos) + ";" 
                                    + "      // CIT_MIT_CIL_ID_INDEX" + "\n" + 
                                "tables.cil[" + (sprintf "%04d" (cilIndexCnt + 1)) + "]" + " <= " + "32'h" + (sprintf "%08X" instDefRec.Instruction.b2) + ";" 
                                    + "      // CIT_MIT_CIL_OPCODE: \t\t" + instDefRec.Instruction.StringName + "\n" + 
                                "tables.cil[" + (sprintf "%04d" (cilIndexCnt + 2)) + "]" + " <= " + "32'h" + (sprintf "%08X" instDefRec.Params) + ";" 
